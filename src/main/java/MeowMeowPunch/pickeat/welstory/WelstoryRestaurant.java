    ...
    public List<ApiTypes.RawMealData> listMealsRaw(int dateYyyymmdd, String mealTimeId) {
        for (int attempt = 0; attempt < 2; attempt++) {
            ...
            try {
                ...
                if (msg != null
                        && (msg.contains("data가 null") || (msg.contains("HTTP 실패") && msg.contains("body=null")))) {
                    log.info("[Welstory][Restaurant] unwrap issue (data-null/body-null) -> force relogin (attempt={})",
                            attempt + 1);
                    if (attempt == 0) {
                        client.http().setAccessToken(null);
                        client.ensureToken();
